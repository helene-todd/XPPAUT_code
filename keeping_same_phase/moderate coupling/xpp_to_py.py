...
with open('from_1.1_to_1.4_beta_0.2.dat', newline='') as file:
    datareader = csv.reader(file, delimiter=' ')
    for row in datareader:
        if float(row[0]) >= 15 and float(row[0]) <= 30  :
        ...
        if float(row[0]) >= 155 and float(row[0]) <= 170  :
...
ax[0, 0].plot(times_plot1, I1_plot1, alpha=0.75, color=c[0], linestyle='-')
ax[0, 0].plot(times_plot1, I2_plot1, alpha=0.75, color=c[1], linestyle='-')
...
ax[0, 1].plot(times_plot2, I1_plot2, alpha=0.75, color=c[0], linestyle='-', label='$I_1$')
ax[0, 1].plot(times_plot2, I2_plot2, alpha=0.75, color=c[1], linestyle='-', label='$I_2$')
...
ax[0, 0].set_ylabel('Current $I_k, k \in \{1,2\}$', size=12)

fig.suptitle('From A/S regime to S regime, $\gamma=0.1, \\beta=0.2$', size=14)
ax[0,1].legend(loc='upper right', bbox_to_anchor=(1, 0.95))
ax[1,1].legend(loc='upper right', bbox_to_anchor=(1, 0.95))

plt.savefig('from_as_to_s_moderate.svg')
plt.show()
