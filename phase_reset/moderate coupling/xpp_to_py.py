...
with open('gamma_0.1_pulse_4.dat', newline='') as file:
    ...
    for row in datareader:
        ...
        if float(row[0]) >= 85 and float(row[0]) <= 100  :
...
fig.suptitle('From A/S regime to A/S regime, $\gamma=0.1, \\beta=0.2$', size=14)
ax[0,1].legend(loc='upper right', bbox_to_anchor=(1, 0.95))
ax[1,1].legend(loc='upper right', bbox_to_anchor=(1, 0.95))

plt.savefig('from_as_to_as_moderate.svg')
#plt.show()
